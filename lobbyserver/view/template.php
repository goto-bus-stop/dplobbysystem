...
// $Edits:	Victor P. $
//------------------------------------------------------
// Template class from phpBB modified to work anywhere.
// Compiles code, and uses cache as well as many other
// Functions.
//------------------------------------------------------
// The following are required when including this class:
// $root_path	= dirname( __FILE__ ) . '/';
// $allow_php	= true;	//true or false
// set them in your file with the same names.
class template
{
	...
	*/
	 ...
	 * @access public
	 */
	function template($root_path = '', $allow_php = false)
	{
	...
	* @access public
	*/
	function set_custom_template($template_path, $template_name = 'default', $template_mode = 'template')
	{
		...
		$this->cachepath = $this->root_path . 'cache/ctpl_' . str_replace('_', '-', $template_name) . '_';
...
class template_compile
{
	...
	*/
	...
	{
		$allow_php = $this->template->allow_php;
		...
		for ($curr_tb = 0, $tb_size = sizeof($blocks); $curr_tb < $tb_size; $curr_tb++)
		{
			...
			switch ($block_val[1])
			{
				...
				case 'INCLUDEPHP':
					$compile_blocks[] = ( $allow_php ) ? '<?php ' . $this->compile_tag_include_php(array_shift($includephp_blocks)) . ' ?>' : '';
				break;

				case 'PHP':
					$compile_blocks[] = ( $allow_php ) ? '<?php ' . array_shift($php_blocks) . ' ?>' : '';
				break;
